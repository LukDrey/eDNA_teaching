...
# cd Filter data on size and quality with NanoFilt
```{bash}

# Count raw sequences
    (cat *.fastq | grep "runid" | wc -l)
    mkdir temporary_filtered
    echo "Filtering data..."
    cat *.fastq | NanoFilt -q 10 -l 500 --maxlength 2000 > temporary_filtered/Q_filt.fastq
...
# Clustering reads to make a list of most abundant, representative reads
    ...
    cd-hit-est -i "$fasta" -o cluster_representatives${fasta%.*} -c 0.8 -n 5 -d 0 -M 0 -T 0 -g 1 > report_"${fasta%.*}.txt"; 
    fi
    done

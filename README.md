...
```{r, eval = F}
overlap <- dplyr::intersect(floristic_soil$Genus, fungi_tax_fin$Genus)
```
...
```{bash, eval = F}
makeblastdb -in full_consensus.fasta -parse_seqids -dbtype nucl -out blastdb_full_consensus
```
